...
if (gameFund <= 0) { alert("لا يوجد رصيد في محفظة التمويل."); return; }
wallet += gameFund;
gameFund = 0;
updateUI();
...
if (isNaN(amt) || amt <= 0 || amt > trxEarned) { alert("مبلغ غير صحيح أو أكبر من أرباحك."); return; }
trxEarned -= amt;
serverWallet += amt;
updateUI();
...
if (!to.startsWith("T") || amt <= 0 || wallet < amt) { alert("تحقق من العنوان أو الرصيد."); return; }
try {
...
if (isNaN(amt) || amt <= 0) { alert("يرجى إدخال مبلغ صالح."); return; }
if (serverWalletUSDT < amt) { alert("لا يوجد رصيد كافٍ في محفظة الخادم."); return; }
serverWalletUSDT -= amt;  
wallet += amt;  
updateUI();  
